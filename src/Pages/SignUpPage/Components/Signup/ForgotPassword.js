...
function ForgotPassword({ open, handleClose }) {
  ...
  const sendOtp = async (e) => {
    ...
    setLoading(true)
    try {
      const response = await axios.post(`${MAIN_URL}admin/reset-password`, { email });
      setOtpSent(true);
  ...
  const verifyOtp = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${MAIN_URL}admin/verify-otp`, { email, otp, new_password: newPassword });
  ...
  const resendOtp = async () => {
    setLoading(true)
    try {
      await axios.post(`${MAIN_URL}admin/resend-otp`, { email });
